...
const Items = () => {
  return (
    <div className="relative h-full py-16">
      <h2 className="text-center text-xl sm:text-2xl text-gray-400 mb-6 sm:mb-8 font-thin">
        PRODUCTS AVAILABLE
      </h2>
...
const ProductCard = ({ item }) => {
  ...
  return (
    <div className="flex flex-col overflow-hidden p-3 sm:p-4 max-w-sm mx-auto">
      <div className="sm:h-72 md:h-96 flex justify-center mb-4">
        <img
          src={selectedImage}
          alt={item.text}
      ...
      <div className="text-center flex flex-col items-center justify-end flex-grow">
        <p className="text-lg sm:text-xl font-light text-gray-700">{item.text}</p>
        <p className="text-md sm:text-lg text-gray-600 font-bold">{item.price}</p>
        <Link
          to="/pages/CreateYours"
          className="mt-3 sm:mt-4 bg-blue-800 py-2 px-4 sm:px-6 rounded-lg text-white font-bold hover:scale-105 transition-transform duration-300"
        >
          Create Now
        </Link>
      </div>
